...
void digram_count(int digram_no, int digram[]){
	...
	while ((str2 = getchar()) != EOF){
		if (str1 >= FIRST_CHAR && str1 < LAST_CHAR && str2 >= FIRST_CHAR && str2 < LAST_CHAR){
			count[(str1 - FIRST_CHAR) * MAX_CHARS + str2 - FIRST_CHAR]++;
		}
		str1 = str2;
	}
	qsort(di_elems, MAX_DIGRAMS, sizeof(int), cmp_di);
	digram[0] = di_elems[digram_no - 1] / MAX_CHARS + FIRST_CHAR;
	digram[1] = di_elems[digram_no - 1] % MAX_CHARS + FIRST_CHAR;
	digram[2] = count[di_elems[digram_no - 1]];
}
